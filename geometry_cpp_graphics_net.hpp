...
#ifndef INCLUDE_GEOMETRY_CPP_GRAPHICS_NET
#define INCLUDE_GEOMETRY_CPP_GRAPHICS_NET
...
{
   template <typename T>
   class geometry_cpp_graphics_net
   {
   public:

      geometry_cpp_graphics_net(Graphics^ gc, const unsigned int& w, const unsigned int& h)
      : gc_    (gc),
        width_ (w),
        height_(h)
      {
     ...
     ~geometry_cpp_graphics_net()
      ...
      inline void draw(const cubic_bezier<T,2>& bezier, const std::size_t& point_count) const
      {
         ...
         geometry_cpp::generate_bezier(bezier, std::back_inserter(point_list), point_count);
