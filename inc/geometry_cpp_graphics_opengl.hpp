#ifndef INCLUDE_GEOMETRY_CPP_GRAPHICS_OPENGL
#define INCLUDE_GEOMETRY_CPP_GRAPHICS_OPENGL
...
namespace geometry_cpp
{
   ...
   template <typename T>
   class geometry_cpp_graphics_opengl
   {
   public:

         geometry_cpp_graphics_opengl(const unsigned int& w, const unsigned int& h, DrawingMode dm)
         : _width(w),
           _height(h),
           drawing_mode(dm),
           red(1.0),
         ...
         {
            geometry_cpp::polygon<T,2> _polygon = make_polygon(make_circle(x,y,radius));
            glColor3fv (color);
            switch (drawing_mode)
            {
         ...
         inline void draw(const cubic_bezier<T,2>& bezier, const std::size_t& point_count)
         {
            ...
            geometry_cpp::generate_bezier(bezier,point_count,point_list);
