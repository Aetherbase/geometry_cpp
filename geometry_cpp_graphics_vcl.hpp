...
#ifndef INCLUDE_GEOMETRY_CPP_GRAPHICS_VCL
#define INCLUDE_GEOMETRY_CPP_GRAPHICS_VCL
...
{
   template <typename T>
   class geometry_cpp_graphics_vcl
   {
   public:

      geometry_cpp_graphics_vcl(TCanvas* c, unsigned int w, unsigned int h)
      : canvas(c),
        _width(w),
        _height(h),
        color(clBlack)
